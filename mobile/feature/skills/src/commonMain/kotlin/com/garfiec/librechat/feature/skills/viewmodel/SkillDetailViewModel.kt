...
sealed interface SkillDetailEvent {
    ...
    data class Exported(val filename: String, val bytes: ByteArray) : SkillDetailEvent
}
...
class SkillDetailViewModel(
    ...
    fun export() {
        ...
        viewModelScope.launch {
            _uiState.value = _uiState.value.copy(isExporting = true, exportProgress = null, error = null)
            when (val result = skillsRepository.exportSkill(skillId) { received, total ->
                ...
                is Result.Success -> {
                    _uiState.value = _uiState.value.copy(isExporting = false, exportProgress = null)
                    _events.emit(SkillDetailEvent.Exported("${skill.name}.skill", result.data))
                }
                is Result.Error -> _uiState.value = _uiState.value.copy(
                    isExporting = false,
                    exportProgress = null,
                    error = result.message ?: "Failed to export skill",
                )
