...
class StreamingManagerLifecycleTest {
    ...
    @Test
    fun `new chat handoff retries until the active stream becomes visible`() =
        runTest(StandardTestDispatcher()) {
            coEvery { chatRepository.checkStreamStatus("conv-1") } returnsMany listOf(
                ChatStatusResponse(active = false),
                ChatStatusResponse(active = false),
                ChatStatusResponse(active = true, conversationId = "conv-1"),
            )
            every { chatRepository.resumeStream("conv-1") } returns emptyFlow()
            val initial = ChatUiState(
                conversation = ConversationMetaState(conversationId = "conv-1"),
                content = MessagesState(isStreaming = false),
            )
            val (delegate, flow) = delegateWith(this, initial)

            delegate.resumeActiveStreamIfNeeded("conv-1", handoffExpected = true)
            assertThat(flow.value.isStreaming).isTrue()
            advanceTimeBy(500)
            runCurrent()

            coVerify(exactly = 3) { chatRepository.checkStreamStatus("conv-1") }
            verify(exactly = 0) { reloadConversation(any()) }
            delegate.reset()
        }

    @Test
    fun `completed handoff reloads messages instead of staying blank`() =
        runTest(StandardTestDispatcher()) {
            coEvery { chatRepository.checkStreamStatus("conv-1") } returns
                ChatStatusResponse(active = false, conversationId = "conv-1")
            val initial = ChatUiState(
                conversation = ConversationMetaState(conversationId = "conv-1"),
                content = MessagesState(isStreaming = false),
            )
