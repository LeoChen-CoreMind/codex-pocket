...
fun MessageList(
    ...
    val streamingProgress = streamingContent.length + streamingTimeline.sumOf { part ->
        part.text.length + (part.toolCall?.output?.length ?: 0) + (part.toolCall?.input?.length ?: 0)
    }
    val currentStreamingProgress by rememberUpdatedState(streamingProgress)
    ...
    val totalItemCount = displayMessages.size + if (isStreaming) 1 else 0
    ...
    val currentOnLoadOlderMessages by rememberUpdatedState(onLoadOlderMessages)
    LaunchedEffect(hasOlderMessages, isLoadingOlderMessages) {
        snapshotFlow {
            hasScrolledToBottom &&
                (isTouching || listState.isScrollInProgress) &&
                listState.firstVisibleItemIndex <= 2
        }.collect { shouldLoad ->
            if (shouldLoad && hasOlderMessages && !isLoadingOlderMessages) {
                currentOnLoadOlderMessages()
            }
        }
    }
    ...
    PullToRefreshBox(
        ...
        LazyColumn(
            ...
            item(key = "chat_inline_content", contentType = "chat_inline_content") {
                LocalChatInlineContent.current()
            }
        }
