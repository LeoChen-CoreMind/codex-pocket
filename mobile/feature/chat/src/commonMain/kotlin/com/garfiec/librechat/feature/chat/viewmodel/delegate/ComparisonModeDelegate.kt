...
class ComparisonModeDelegate(
    ...
    fun onSendStart() {
        ...
        handle.update {
            comparisonState = comparisonState.copy(
                ...
                primaryStreamingTimeline = emptyList(),
                secondaryStreamingTimeline = emptyList(),
                primaryAgentId = null,
    ...
    fun routeEvent(event: StreamEvent): Boolean {
        ...
        return when (event) {
            ...
            is StreamEvent.ContentDelta -> {
                routeTextDelta(event.agentId, event.chunk, StreamingPartType.TEXT)
                true
            }
            is StreamEvent.ThinkingDelta -> {
                routeTextDelta(event.agentId, event.chunk, StreamingPartType.THINKING)
                true
            }
    ...
    private fun routeTextDelta(agentId: String?, chunk: String, type: StreamingPartType) {
        if (isSecondaryEvent(agentId)) {
            secondaryBuffer.append(chunk)
            handle.update {
                comparisonState = comparisonState.copy(
                    secondaryStreamingContent = secondaryBuffer.toString(),
                    secondaryStreamingTimeline = appendTextPart(
                        comparisonState.secondaryStreamingTimeline,
                        type,
                        chunk,
                    ),
                    secondaryIsStreaming = true,
            ...
            handle.update {
                comparisonState = comparisonState.copy(
                    ...
                    primaryStreamingTimeline = appendTextPart(
                        comparisonState.primaryStreamingTimeline,
                        type,
                        chunk,
                    ),
                    primaryIsStreaming = true,
        ...
        if (isSecondaryEvent(event.agentId)) {
            handle.update {
                comparisonState = comparisonState.copy(
                    ...
                    secondaryStreamingTimeline = comparisonState.secondaryStreamingTimeline +
            ...
            handle.update {
                comparisonState = comparisonState.copy(
                    ...
                    primaryStreamingTimeline = comparisonState.primaryStreamingTimeline +
                        StreamingTimelinePart(
                            id = "tool:${event.toolCallId}",
                            type = StreamingPartType.TOOL,
                            toolCall = newToolCall,
        ...
        if (isSecondaryEvent(event.agentId)) {
            handle.update {
                ...
                comparisonState = comparisonState.copy(secondaryActiveToolCalls = updated)
                    .copy(
                        secondaryStreamingTimeline = completeToolPart(
                            comparisonState.secondaryStreamingTimeline,
                            event.toolCallId,
                            event.output,
                        ),
            ...
            handle.update {
                ...
                comparisonState = comparisonState.copy(
                    primaryActiveToolCalls = updated,
                    primaryStreamingTimeline = completeToolPart(
                        comparisonState.primaryStreamingTimeline,
                        event.toolCallId,
                        event.output,
                    ),
    ...
    private fun completeToolPart(
        parts: List<StreamingTimelinePart>,
        toolCallId: String,
        output: String,
    ): List<StreamingTimelinePart> = parts.map { part ->
        if (part.type == StreamingPartType.TOOL && part.toolCall?.id == toolCallId) {
            part.copy(toolCall = part.toolCall.copy(isComplete = true, output = output))
        } else {
            part
        }
    }
    ...
    fun endStreaming(clearContent: Boolean = false) {
        ...
        handle.update {
            comparisonState = comparisonState.copy(
                ...
                primaryActiveToolCalls = emptyList(),
                secondaryActiveToolCalls = emptyList(),
                primaryStreamingTimeline = if (clearContent) emptyList() else comparisonState.primaryStreamingTimeline,
                secondaryStreamingTimeline = if (clearContent) emptyList() else comparisonState.secondaryStreamingTimeline,
