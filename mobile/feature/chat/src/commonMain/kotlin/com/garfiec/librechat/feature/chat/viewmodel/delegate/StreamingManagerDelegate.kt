...
class StreamingManagerDelegate(
    ...
    private var nextTimelinePartId = 0L
    ...
    private fun handleStreamEvent(event: StreamEvent) {
        ...
        when (event) {
            ...
            is StreamEvent.ContentDelta -> {
                ...
                appendTimelineText(StreamingPartType.TEXT, event.chunk)
            }
            is StreamEvent.ThinkingDelta -> {
                streamingBuffer.append(event.chunk)
                streamingBufferDirty = true
                appendTimelineText(StreamingPartType.THINKING, event.chunk)
            }
            ...
            is StreamEvent.ToolCallStart -> {
                ...
                handle.update {
                    val existingIndex = content.streamingTimeline.indexOfFirst { part ->
                        part.type == StreamingPartType.TOOL && part.toolCall?.id == event.toolCallId
                    }
                    val timeline = if (existingIndex >= 0) {
                        content.streamingTimeline.mapIndexed { index, part ->
                            if (index == existingIndex) part.copy(toolCall = newToolCall) else part
                        }
                    } else {
                        content.streamingTimeline + StreamingTimelinePart(
                    ...
                    content = content.copy(
                        activeToolCalls = content.activeToolCalls
                            .filterNot { it.id == event.toolCallId } + newToolCall,
            ...
            is StreamEvent.ToolCallComplete -> {
                handle.update {
                    val existing = content.activeToolCalls.firstOrNull { it.id == event.toolCallId }
                    val completed = (existing ?: ActiveToolCall(
                        id = event.toolCallId,
                        name = "operation",
                    )).copy(isComplete = true, output = event.output)
                    val toolIndex = content.streamingTimeline.indexOfFirst { part ->
                        part.type == StreamingPartType.TOOL && part.toolCall?.id == event.toolCallId
                    }
                    val timeline = if (toolIndex >= 0) {
                        content.streamingTimeline.mapIndexed { index, part ->
                            if (index == toolIndex) part.copy(toolCall = completed) else part
                        }
                    } else {
                        content.streamingTimeline + StreamingTimelinePart(
                            id = "tool:${event.toolCallId}",
                    ...
                    content = content.copy(
                        activeToolCalls = content.activeToolCalls
                            .filterNot { it.id == event.toolCallId } + completed,
                        streamingTimeline = timeline,
                    )
                }
            ...
            is StreamEvent.Sync -> {
                ...
                val syncedTimeline = event.aggregatedContent.mapIndexedNotNull { index, part ->
                    val text = part.text
                    val thinking = part.think
                    val rawToolCall = part.toolCall
                    when {
                        text != null -> StreamingTimelinePart(
                            id = "sync:text:$index",
                            type = StreamingPartType.TEXT,
                            text = text,
                        )
                        thinking != null -> StreamingTimelinePart(
                            id = "sync:think:$index",
                            type = StreamingPartType.THINKING,
                            text = thinking,
                        )
                        rawToolCall != null -> {
                            val toolCall = syncedToolCalls.firstOrNull { it.id == rawToolCall.id }
                                ?: return@mapIndexedNotNull null
                            StreamingTimelinePart(
                                id = "tool:${toolCall.id}",
                                type = StreamingPartType.TOOL,
                                toolCall = toolCall,
                            )
                ...
                nextTimelinePartId = syncedTimeline.size.toLong()
                handle.update {
                    content = content.copy(
                        activeToolCalls = syncedToolCalls,
                        streamingTimeline = syncedTimeline,
                    )
                }
    ...
    private fun appendTimelineText(type: StreamingPartType, chunk: String) {
        if (chunk.isEmpty()) return
        handle.update {
            val parts = content.streamingTimeline
            val last = parts.lastOrNull()
            val updated = if (last?.type == type) {
                parts.dropLast(1) + last.copy(text = last.text + chunk)
            } else {
                parts + StreamingTimelinePart(
                    id = "stream:${nextTimelinePartId++}",
                    type = type,
                    text = chunk,
                )
            }
            content = content.copy(streamingTimeline = updated)
        }
    }
    ...
    fun resumeActiveStreamIfNeeded(conversationId: String, handoffExpected: Boolean = false) {
        ...
        val session = streamSession
        if (handoffExpected) {
            // The Created frame navigates before the server has necessarily published the
            // conversation-id -> active-job mapping queried below. Keep the handed-off turn live
            // while that mapping catches up; otherwise one early inactive response leaves the new
            // screen blank until the user reopens it from history.
            handle.update {
                content = content.copy(
                    isStreaming = true,
        ...
        scope.launch {
            val attempts = if (handoffExpected) HANDOFF_RESUME_ATTEMPTS else 1
            ...
            repeat(attempts) { attempt ->
                ...
                if (attempt < attempts - 1) delay(HANDOFF_RESUME_RETRY_DELAY_MS)
            }

            if (isResumeStale(session) || abortRequested || handle.state.remoteTurnActive) return@launch
            lastFailure?.let { error ->
                Logger.d(error) { "Handed-off stream was not available for $conversationId" }
            }
            // The turn may have completed between Created and the first successful status read.
            // End the provisional streaming state and force the normal network-backed message
            // reload so the completed response appears without a history-list round trip.
            handle.update { content = content.copy(isStreaming = false) }
    ...
    private companion object {
        ...
        /** Covers the short Created -> active-job lookup publication race during route handoff. */
        const val HANDOFF_RESUME_ATTEMPTS = 20
        const val HANDOFF_RESUME_RETRY_DELAY_MS = 250L
