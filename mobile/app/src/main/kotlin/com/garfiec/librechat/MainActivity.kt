...
import com.garfiec.librechat.approval.ApprovalInlineContent
import com.garfiec.librechat.approval.McpDialogGlobalOverlay
import com.garfiec.librechat.approval.ApprovalService
import com.garfiec.librechat.feature.chat.components.LocalChatInlineContent
...
class MainActivity : ComponentActivity() {
    ...
    private val activeAccountProvider: ActiveAccountProvider by inject()
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                activeAccountProvider.state
                    .map { state -> (state as? AccountState.Resolved)?.id != null }
                    .distinctUntilChanged()
                    .collect { loggedIn ->
                        if (loggedIn) ApprovalService.start(this@MainActivity)
                        else ApprovalService.stop(this@MainActivity)
                    }
            }
        }
                    ...
                    Surface(
                        ...
                        Box(modifier = Modifier.fillMaxSize()) {
                            CompositionLocalProvider(
                                LocalChatInlineContent provides { ApprovalInlineContent() },
                            ) {
