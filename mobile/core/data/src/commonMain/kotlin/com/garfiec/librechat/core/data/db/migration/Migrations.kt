...
/**
 * Removes legacy cache payloads that exceed Android's CursorWindow row limit. New writes are
 * bounded structurally in MessageMapper; existing JSON cannot be safely substring-truncated, so
 * oversized structured content is discarded while the plain response text is retained.
 */
val MIGRATION_8_9: Migration = object : Migration(8, 9) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("UPDATE messages SET content = NULL WHERE length(content) > 524288")
        connection.execSQL(
            "UPDATE messages SET text = substr(text, 1, 262144) " +
                "WHERE length(text) > 262144",
        )
        connection.execSQL("UPDATE messages SET metadata = NULL WHERE length(metadata) > 65536")
    }
}
