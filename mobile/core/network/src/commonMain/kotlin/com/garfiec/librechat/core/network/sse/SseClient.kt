            ...
            try {
                ...
                when (e.statusCode) {
                    HttpStatusCode.NotFound.value -> {
                        // A new-chat navigation hand-off can reconnect while the bridge is
                        // rebuilding its compatibility stream. Treat 404 as transient instead of
                        // silently completing the Flow and leaving the current screen stale.
                        attempt++
                        if (attempt > maxRetries) {
                            emit(StreamEvent.Error(message = "Chat stream is not available.", code = "404"))
                            done = true
                        }
                    }
